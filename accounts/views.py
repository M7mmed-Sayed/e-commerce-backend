...
class UserLoginView(CreateAPIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid()==False :
            return Response({'error': 'Invalid credentials Invalid Email or Password'}, status=status.HTTP_401_UNAUTHORIZED)
        response = {
...
class UserUpdateView(UpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        if user.username != instance.username:
            return Response({"authorize": "You don't have permission for this user."}, status=status.HTTP_401_UNAUTHORIZED)
